...
goto SKIP_MY_LIB
:: MyLib
cd %Project_Path%\lib_proj\My_Static_Method_Lib
rmdir /s /q gen
rmdir /s /q bin
mkdir gen
mkdir bin
...
if errorlevel 1 goto ERROR
cd %Project_Path%\lib_proj\dslv_library
rmdir /s /q gen
rmdir /s /q bin
mkdir gen
mkdir bin
...
@if exist C:\Windows\System32\my_notpad\notepad.exe ( call C:\Windows\System32\my_notpad\notepad.exe %Compile_Info_Full_Path%\%Compile_Info_File_Name%) else ( start %Compile_Info_Full_Path%\%Compile_Info_File_Name%)
goto ERROR
